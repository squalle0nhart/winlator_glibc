    ...
    VkRect2D scissorRect)
{
    ...
    bool useSgsr    = (filterMode == 2 || filterMode == 6)
...
void VulkanRendererContext::dumpRendererInfo() {
    ...
    __android_log_print(ANDROID_LOG_DEBUG,WLOG_TAG,
        "Filter: mode=%d (%s)",
        filterMode, filterMode==6?"SGSR_HQ":filterMode==5?"COLOR_BOOST":filterMode==3?"FSR1":filterMode==2?"SGSR":filterMode==1?"NEAREST":"LINEAR");
    __android_log_print(ANDROID_LOG_DEBUG,WLOG_TAG,
...
void VulkanRendererContext::setFilterMode(int mode) {
    auto modeName=[](int m){ return m==1?"NEAREST":m==2?"SGSR":m==3?"FSR1":m==5?"COLOR_BOOST":m==6?"SGSR_HQ":"LINEAR"; };
    ...
    if (mode == 6 && sgsrQualityPipeline == VK_NULL_HANDLE) createSgsrQualityPipeline();
    vk_.DeviceWaitIdle(device);
